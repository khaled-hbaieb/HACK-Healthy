...
const isAdmin = async function(next) {
  ...
  let user = await axios.post("/api/users/clinicX/administrators", {
    _id: token.userId,
  });
...
const isPatient = async function(next) {
  ...
  let user = await axios
    .post("/api/users/clinicX/patients/findPatients", { _id: token.userId })
...
const isDoctor = async function(next) {
  let token = jwtDecode(localStorage.getItem("token"));
  let user = await axios
    .post("/api/users/clinicX/doctors/findDoctors", { _id: token.userId })
    .catch((err) => console.log(err));
  if (user.data.length > 0) {
    next();
  } else {
