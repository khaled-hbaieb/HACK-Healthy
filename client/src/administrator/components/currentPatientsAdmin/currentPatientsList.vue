        ...
        <vs-card vs-xs="12" vs-sm="12" vs-lg="12" id="profile" actionable class="card">
          <div slot="media">
            <a href="/patients/patient">
              <img id="current-patient-profile-image" :src="currentPatient.imageName" alt="user" />
            </a>
          </div>
          <div id="footer" slot="footer">
            ...
            <br />
            <small>Date Of Birth: {{currentPatient.dateOfBirth}}</small>
            <br />
            <small>Room: {{currentPatient.room}}</small>
          </div>
        </vs-card>
      </vs-col>
    </vs-row>
...
export default {
  ...
  async beforeMount() {
    let currentPatients = await axios.post(
      "api/users/clinic/currentPatients",
      {}
    );
    thiscurrentPatients = currentPatientsdata;
  },
};
</script>
<style scoped>
